...
function authorize(req, authConfig, requiredScope) {
  ...
  if (!token) {
    return { ok: false, statusCode: 401, code: 'AUTH_REQUIRED', message: 'Missing Bearer token', details: { header: 'Authorization: Bearer <token>', required_scope: requiredScope || null } };
  }

  const scopes = authConfig.tokens.get(token);
  if (!scopes) {
    return { ok: false, statusCode: 401, code: 'AUTH_INVALID', message: 'Invalid API token', details: { required_scope: requiredScope || null } };
  }
  ...
  if (!hasScope(scopes, requiredScope)) {
    return {
      ...
      details: { required_scope: requiredScope, token_scopes: Array.from(scopes) },
    };
  }
